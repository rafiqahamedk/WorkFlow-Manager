  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center gap-3 bg-slate-800/50 border border-slate-700/30 rounded-lg px-3 py-2.5 group">
      ...
      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button type="button" onClick={() => onEdit(rule)} className="text-violet-400 hover:text-violet-300 text-xs px-2 py-1 rounded bg-violet-400/10 transition-colors">Edit</button>
        <button type="button" onClick={() => onDelete(rule._id)} className="text-red-400 hover:text-red-300 text-xs px-2 py-1 rounded bg-red-400/10 transition-colors">✕</button>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div className="bg-slate-800/30 border border-slate-700/30 rounded-xl p-4 space-y-3">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          ...
          {editingId && (
            <button type="button" onClick={() => { setEditingId(null); setForm(emptyRule); }} className="text-slate-500 hover:text-slate-300 text-sm transition-colors">Cancel</button>
          )}
        </div>
