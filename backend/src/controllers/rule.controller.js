...
import { evaluateCondition } from '../engine/ruleEngine.js';

/** Validate condition syntax by doing a dry-run with empty data */
...
export async function addRule(req, res, next) {
  try {
    ...
    const syntaxError = validateConditionSyntax(req.body.condition);
    if (syntaxError) return res.status(400).json({ error: `Invalid condition syntax: ${syntaxError}` });
